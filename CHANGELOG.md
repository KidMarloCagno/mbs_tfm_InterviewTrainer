...
- Added new question sets `Design Patterns`, `Git Basics`, `Kafka Basics`, and `Maven Basics`, and registered them in `lib/questions-data.ts` so they appear in dashboard practice topics.
...
- Added new question sets `CI/CD Basics`, `DevOps Basics`, and `Spring Boot Basics` and registered them in `lib/questions-data.ts`.
- Registered all new dataset files so they are immediately available in the dashboard without a DB re-seed.
